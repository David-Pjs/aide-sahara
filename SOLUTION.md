...
- **Asked once, changeable by voice.** Sahara has no auto-detect, so on first visit Aide asks: *"Which language do you speak? English, Pidgin, Yoruba, Igbo, or Hausa."* It remembers the answer, and saying "Naija", "Yoruba", "Igbo" or "Hausa" at any point switches it. The switch also accepts the ways a recogniser mishears those names ("Pidgin" once came back as "peagon"), because a language switch that cannot be spoken can never be turned on.
- **Understands the mix, answers in a voice that can say it.** Aide hears Pidgin, Yoruba, Igbo and Hausa mixed with English, and always replies in clear English. No neural voice we could ship pronounces Yoruba, Igbo or Hausa well (Microsoft has none for these languages, and Sahara TTS took about 7 seconds a sentence on the live site), and a garbled reply is worse for a blind user than an English one. Names and amounts are kept exactly as spoken.
