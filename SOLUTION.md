...
> **About the live demo.** The live site listens through Sahara. Every browser records the utterance and sends it to `/api/stt`, so it works in Chrome, Safari, Firefox, Brave and on iPhone and Android alike. If Sahara cannot answer (no credit, a timeout, an outage), the same request falls through to Whisper hosted on Groq, so a user is still heard; the fallback hears code-switched speech worse, which [`ETHICS.md`](ETHICS.md) states plainly.
